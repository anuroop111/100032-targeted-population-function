...
def hello_name():
    if request.method == 'POST':
        ...
        for i in range(0, S):
            ...
            if d == 5:
                stage['period'] = stages_form_data['period'][0]
        ...
        if isinstance(targeted_population, pandas.DataFrame):
            ...
            describe_html = targeted_population.agg(
                {'C/10001': ['sum', 'mean', 'std'], 'B/10002': ['sum', 'mean', 'std', ],
                 'C/10003': ['sum', 'mean', 'std', ], 'D/10004': ['sum', 'mean', 'std', ]}).to_html()
            resp = make_response(
                render_template_string(df_html + '<br><br>' + describe_html + '<br><br>' + status_html))
            return resp
